  ...
  private validateSchema = async (
    ...
    try {
      ...
      return res
        .status(400)
        .type('json')
        .send(new ErrorResponse('Invalid JSON format for architecture'));
    }
    ...
    try {
      ...
      return res
        .status(500)
        .type('json')
        .send(new ErrorResponse('Failed to load schemas'));
    }
    let foundSchema;
    try {
      ...
      return res
        .status(500)
        .type('json')
        .send(new ErrorResponse('Failed to load schema: ' + err));
    }
    try {
